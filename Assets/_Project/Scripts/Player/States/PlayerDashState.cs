...
public class PlayerDashState : PlayerState
{
    ...
    public override void Enter()
    {
        ...
        // 진입 프레임에 마나 소모 시도
        if (player.Mana != null && player.Mana.UseMana(player.Data.dashManaCost))
        {
            player.SetDashInvincibility(true);
            player.UseDash(); // 대시 입력 소비 및 쿨타임 초기화

            dashTimer = player.Data.dashDuration; // 타이머 설정

            // 방향 결정: 키보드를 누르고 있으면 그 방향, 안 누르고 있으면 현재 바라보는 방향
            if (player.MoveInput.x != 0)
                dashDirection = Mathf.Sign(player.MoveInput.x);
            else
                dashDirection = player.Movement.FacingDirection;
    ...
    public override void FixedUpdate()
    {
        ...
        player.Movement.SetVelocity(dashDirection * player.Data.dashSpeed, 0f);
    }
    ...
    public override void Exit()
    {
        ...
        player.SetDashInvincibility(false);
