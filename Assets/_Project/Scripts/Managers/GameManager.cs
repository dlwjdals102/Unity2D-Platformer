...
public class GameManager : MonoBehaviour
{
    ...
    private void RestoreSession()
    {
        ...
        var data = DataManager.Instance.sessionData;

        // [안전 점검 1] 플레이어 데이터가 잘 할당되어 있는지 확인
        if (player.Health == null)
        {
            Debug.LogError("[GameManager] 에러: Player의 HealthComponent가 없습니다!");
            return;
        }
        if (player.Data == null)
        {
            Debug.LogError("[GameManager] 에러: Player의 Data (ScriptableObject)가 인스펙터에 비어있습니다!");
            return;
        }

        // 플레이어 체력 복구
        player.Health.Initialize(data.maxHealth, player.Data.iFrameDuration);
